...
XVIDEO_VIEW_API long long NowMs();

class AVFrame;
class XVIDEO_VIEW_API XVideoView
{
public:
	enum PixFormat // 顺序与FFmepeg相同 AVPixelFormat
	{
		YUV420P = 0,
		RGB = 2,
		NV12 = 23,
	...
	static XVideoView* Create(int w, int h, void* win_id, int fmt, ViewType type = SDL);// 创建并init
	...
	// 绘制
	bool Draw();
	bool Draw(unsigned char* data, int linesize);
	bool Draw(
...
protected:
	...
	virtual bool DrawFrame(unsigned char *data, int linesize) = 0;
	virtual bool DrawFrame(
		unsigned char* data_y, int linesize_y,
		unsigned char* data_u, int linesize_u,
		unsigned char* data_v, int linesize_v) = 0;
protected:
	int scale_w_;
	int scale_h_;
	std::ifstream ifs_;
