...
int main()
{
	...
	// 打开解码器
	re = avcodec_open2(context,NULL,NULL);
	if (re < 0) 
	{
		char buf[128];
		av_strerror(re,buf,sizeof(buf));
			...
			while (true)
			{
				...
				if (re == 0) {
					...
					if (is_first)
					{
						...
						// 创建窗口、渲染器、材质
						window_ = SDL_CreateWindow("", p_frame->width, p_frame->height, SDL_WINDOW_RESIZABLE);
					...
					if (ret == 0) {
						...
						/*	
							...
							U2	V2	U3	V3
						*/
						texture_ = SDL_CreateTexture(render_, SDL_PIXELFORMAT_NV12, SDL_TEXTUREACCESS_STREAMING, width, height);
						...
						SDL_UpdateTexture(texture_,NULL, cache.get(), p_frame->width);
					}
					else {
